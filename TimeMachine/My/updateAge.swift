...
import Foundation
import UIKit
class updateAge: UIViewController {
    
    var updateAgeLabel:UITextField!          //修改名字输入框
    ...
    func pageLayout(){
        //输入框
        updateAgeLabel=UITextField()
        
        updateAgeLabel.frame=CGRectMake(0, 80, screen.width, 40)
        
        updateAgeLabel.addTarget(self, action: "updataNick:", forControlEvents: UIControlEvents.EditingChanged)
        updateAgeLabel.keyboardType=UIKeyboardType.NumberPad
        updateAgeLabel.text=memberEntity?.age
        updateAgeLabel.clearButtonMode=UITextFieldViewMode.Always
        updateAgeLabel.backgroundColor=UIColor.whiteColor()
        self.view.addSubview(updateAgeLabel)
        //保存按钮
        let saveBtn:UIButton=UIButton()
        saveBtn.frame=CGRectMake(0, CGRectGetMaxY(updateAgeLabel.frame)+30, screen.width, 40)
